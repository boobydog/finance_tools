...
| `interested`(候補) | 候補スクリーニング(系統A/B)が自動付与、または手動で設定。買入タイミング判定の対象。 |
| `considering`(検討) | ユーザーが「買う前提でじっくり検討したい」と手動でマークした状態。`interested`とは独立した手動ステータスで、候補スクリーニングの自動評価対象外(status IS NULLの銘柄のみが評価対象のため)。買入タイミング判定の対象。 |
