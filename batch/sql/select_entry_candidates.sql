-- 買入タイミング判定画面向け: 候補(interested)・検討(considering、ユーザーが購入前提で
-- 手動マークした銘柄)に加え、複数回の買い増し・再エントリーの判断にも使えるよう
-- 保有中(holding)・売却済(sold)銘柄もエントリーシグナル関連指標を返す
-- (除外(excluded)・未設定は対象外)。
SELECT
    s.ticker_symbol,
    s.name,
...
WHERE u.status IN ('interested', 'holding', 'sold', 'considering')
